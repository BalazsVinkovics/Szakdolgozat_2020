...
prediction_0_20 = [1, 0 ,0 ,0]
prediction_20_40 = [0, 1 ,0 ,0]
prediction_40_60 = [0, 0 ,1 ,0]
prediction_60_80 = [0, 0 ,0 ,1]
...
for emb in dataset.iloc():
        ...
        if 0 <= emb[128]:
            if emb[128] <= 20:
                emb.append(prediction_0_20)
        if 21 <= emb[128]:
            if emb[128] <= 40:
                emb.append(prediction_20_40)
        if 41 <= emb[128]:
            if emb[128] <= 60:
                emb.append(prediction_40_60)
        if 61 <= emb[128]:
            if emb[128] <= 80:
                emb.append(prediction_60_80)
...
for emb in training_dataset:
    Y_train_label.append(emb[131])
Y_train_label = np.asarray(Y_train_label)
    
for emb in test_dataset:
    Y_test_label.append(emb[131])
...
history = model.fit(X_train_emb, Y_train_label, epochs=50, validation_split=0.2, steps_per_epoch=20)
...
model.save("D:/Szakdolgozat/Neural_Networks3/DNN_race")

#Plot results
plt.plot(history.history['accuracy'])
plt.plot(history.history['val_accuracy'])
plt.title('A kor modell pontossága')
